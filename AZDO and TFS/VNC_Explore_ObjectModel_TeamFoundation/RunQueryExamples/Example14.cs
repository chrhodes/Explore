...
namespace WordToTFS
{
    ...
    public class TfsManager
    {
        ...
        {
            collection = new TfsTeamProjectCollection(new Uri(string.Format(@"{0}/{1}", connectUri, collectionName)), Credential);
        ...
        public string GetWorkItemLink(int id, string projectName)
        {
            switch (tfsVersion)
            {
                case TfsVersion.Tfs2011:
                    ...
                    return string.Format(@"{0}/{1}/_workitems/edit/{2}", ItemsStore.TeamProjectCollection.Uri, projectName, id);
                case TfsVersion.Tfs2010:
