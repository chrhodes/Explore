...
using ChipworkXPhidgets.Events;

namespace ChipworkXPhidgets
{
    public class Phidget1018
    {
        ...
        private void DeviceConnectedEvent(USBH_Device device)
        {
            ...
            if (device.TYPE == USBH_DeviceType.HID && device.VENDOR_ID == 0x06C2 && device.PRODUCT_ID == 0x0045)  // Detect only Phidgets' InterfaceKit
            {
                ...
                USBH_Descriptors.Configuration cd = _IF1018.GetConfigurationDescriptors(0);   // Get descriptors
                USBH_Descriptors.Endpoint _IF1018_EP = null; // communication endpoint
                USBH_ERROR e = 0;
                    ...
                    if (cd.interfaces[i].bInterfaceClass == 0x03)  // found
                    {
                        for (int ep = 0; ep < cd.interfaces[i].endpoints.Length; ep++)  // look for input interrupt Endpoint
                        {
                            if (cd.interfaces[i].endpoints[ep].bmAttributes == 0x03)  // is it interrupt Endpoint?
                            {
                                ...
                                byte[] toto = new byte[0x0E];
                                _IF1018.SendSetupTransfer(0x80, 0x06, 0x0303, 0x0409, toto, 0, 0x0E);
                                for (int j = 2; j <= 0x0C; j += 2) SerialNumber += (char)toto[j];
                                #endregion
                                ...
                                try
                                {
                                    ...
                                    byte[] tutu = new byte[0x20];
                                    _IF1018.SendSetupTransfer(0x80, 0x06, 0x0302, 0x0409, tutu, 0, 0x20);
