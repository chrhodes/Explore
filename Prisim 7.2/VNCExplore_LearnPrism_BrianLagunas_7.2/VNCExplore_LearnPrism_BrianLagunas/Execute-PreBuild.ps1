...
#TODO - Get this from env
    ...
    $targets = @(
	    ".\bin\$Configuration\Modules"
	    ".\bin\$Configuration\ModulesAppConfig"
		".\bin\$Configuration\ModulesDynamic"
	    )

	foreach ($target in $targets)
	{
		"Checking if $target exits"
		
		if (-not (Test-Path -LiteralPath $target))
		{
			try
			{
				New-Item -Path $target -ItemType Directory -ErrorAction Stop | Out-Null #-Force
			}
			catch
			{
				Write-Error -Message "Unable to create Directory '$target'.  Error was: $_"
				Exit
			}
