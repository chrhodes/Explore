        ...
        private class InnerModuleInfoLoader : MarshalByRefObject
        {
            ...
            internal void LoadAssemblies(IEnumerable<string> assemblies)
            {
                ...
                foreach (string assemblyPath in assemblies)
                {
                    try
                    {
                        VNC.Log.Trace($"Loading {assemblyPath}", Common.LOG_APPNAME, 0, startTicks);
                        Assembly.ReflectionOnlyLoadFrom(assemblyPath);
                    }
                    catch (FileNotFoundException)
                    {
                        VNC.Log.Info($"Ex FileNotFound {assemblyPath}", Common.LOG_APPNAME, 0, startTicks);
            ...
            {
                long startTicks = VNC.Log.Trace($"Enter: type{type.Name}", Common.LOG_APPNAME, 0);
